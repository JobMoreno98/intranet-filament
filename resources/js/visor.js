...
if (currentPage <= 0) return; await renderPage(currentPage - 1); } //=========================// KEYBOARD
    ...
    // swipe horizontal
    if (Math.abs(deltaX) < 50) { return; } // no cambiar página si zoom if (panzoom.getScale()> 1.05) {
        return;
        }

        if (deltaX < 0) { await nextPage(); } else { await prevPage(); } }); //=========================// DOBLE CLICK
            ZOOM //=========================viewer.addEventListener("dblclick", ()=> {
            panzoom.zoomIn();
            });
