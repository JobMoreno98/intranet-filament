...
<body class="overflow-hidden">
    ...
    <script type="module">
        ...
        const lightbox = new PhotoSwipeLightbox({
            ...
            secondaryZoomLevel: 2,
            maxZoomLevel: 4,
            counter: false, // Desactivamos el contador nativo para usar el nuestro
        });

        // --- INYECCIÓN DE INTERFAZ PERSONALIZADA ---
        lightbox.on('uiRegister', function() {
            // 1. Agregar Título de la Obra al Visor
            ...
            // 2. Agregar Selector de Página al Visor
            lightbox.pswp.ui.registerElement({
                name: 'custom-pager',
                order: 10,
                isOrigin: false,
        ...
        // Sincronizar el input con el cambio de página
        lightbox.on('change', () => {
            const input = document.getElementById('pswp-input-page');
            if (input) {
                input.value = lightbox.pswp.currIndex + 1;
            }
        });

        // Permitir saltar de página escribiendo en el input
        document.addEventListener('change', (e) => {
            if (e.target.id === 'pswp-input-page') {
                const page = parseInt(e.target.value) - 1;
                if (page >= 0 && page < {{ count($paginas) }}) {
                    lightbox.pswp.goTo(page);
                }
            }
        });
